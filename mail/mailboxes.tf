...
resource "migadu_identity" "extra" {
  ...
  may_access_imap         = each.value.may_access_imap == null ? false : each.value.may_access_imap
  may_access_manage_sieve = each.value.may_access_manage_sieve == null ? false : each.value.may_access_manage_sieve
  may_access_pop3         = each.value.may_access_pop3 == null ? false : each.value.may_access_pop3
